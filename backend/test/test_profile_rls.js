...
async function runTest() {
  ...
  const instEmail = `inst_${timestamp}@example.com`;
  ...
  try {
    ...
    console.log('Query succeeded. Requests returned count:', queriedReqs.length);

    console.log('9. Querying individual user profile from profiles table...');
    const { data: queriedProfile, error: profileQueryErr } = await supabase
      .from('profiles')
      .select('*')
      .eq('id', userId)
      .maybeSingle();
    ...
    if (profileQueryErr) {
      console.error('Profile query failed:', profileQueryErr);
    } else {
      console.log('Profile query result:', queriedProfile);
      if (!queriedProfile) {
        console.log('❌ PROFILE FETCH RETURNED NULL (RLS restriction is active!)');
      } else {
        console.log('✅ PROFILE FETCH SUCCEEDED (No RLS restriction, or matching policy allows it)');
      }
    }
